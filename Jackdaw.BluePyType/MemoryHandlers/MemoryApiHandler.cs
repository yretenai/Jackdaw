...
namespace Jackdaw.BluePyType.MemoryHandlers;
...
public sealed partial class MemoryApiHandler : IMemoryHandler {
	...
	public unsafe bool ReadBytes(nint address, Span<byte> buffer, out int bytesRead) {
		fixed (byte* pin = buffer) {
			if (!NativeMethods.ReadProcessMemory(Process.SafeHandle, address, (nint) pin, buffer.Length, out var read)) {
				bytesRead = 0;
				return false;
			}
	...
	private static partial class NativeMethods {
		...
		internal static partial bool ReadProcessMemory(SafeProcessHandle processHandle, nint address, nint bytes, nint size, out nint bytesReadCount);
	}
}
